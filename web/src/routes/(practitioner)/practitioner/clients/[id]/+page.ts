...
export async function load({ fetch, params }) {
	const [client, sessions, documents, intake, files, entries] = await Promise.all([
		...
		get(fetch, `/me/clients/${params.id}/entries`).catch(() => [])
	]);
	return { client, sessions, documents, intake, files, entries };
}
