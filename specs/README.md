...
|---|---|---|
| [v6.1](v6.1/README.md) | **investigated, not implemented** | Ingestion latency audit, prompted by the same-day RETRIEVAL_MODEL/REASONER_MODEL reasoning-token bugs. Ingestion has no reasoning-token problem (Reader/Graph-builder both confirmed non-reasoning), but does have two real, undocumented latency sources: a sequential O(chunks) Graph-builder loop (~26s/10 passages) and high fixed per-call Embedder latency (~9.5s/call) — both timed live, neither fixed, documented for a future decision |
