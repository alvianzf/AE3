...
|---|---|---|
| [v4.2](v4.2/README.md) | **spec only, not built** | Swap the LLM provider from Anthropic to Nebius's OpenAI-compatible token factory, splitting the current 4-role pipeline into 6 (Reader, graph-builder, embedder, retrieval-planner, reasoner, checker) — includes two unresolved open questions (the practitioner-owned-key model, and what consumes the new embedder's output) that block implementation |
