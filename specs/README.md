...
|---|---|---|
| [v6.3](v6.3/README.md) | **investigated, one candidate prototyped and rejected** | Checker alternatives, triggered by `run_check` defaulting to `false` in production (2026-09-14) because HHEM-2.1-Open still costs 28-77s/call even after the OOM fix. Evaluated four candidates; the recommended first prototype (MiniCheck-RoBERTa-Large) was built and tested same-day, came back **worse** (185.21s/call, 2.5GB peak) — reverted, nothing shipped. Points at CPU-only inference as the real bottleneck, not model choice; small-LLM-as-judge via the existing `Role` pattern is the remaining unprototyped candidate |
