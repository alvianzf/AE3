...
|---|---|---|
| [v6.3](v6.3/README.md) | **investigated, not implemented** | Checker alternatives, triggered by `run_check` defaulting to `false` in production (2026-09-14) because HHEM-2.1-Open still costs 28-77s/call even after the OOM fix. Evaluated four candidates against the current `check()` contract: ONNX-exported HHEM, MiniCheck-RoBERTa-Large (recommended first prototype), a generic small NLI cross-encoder, and a small-LLM-as-judge fallback via the existing `Role` pattern (recommended as the true last-measure option) |
